...
# Obtenção da série dessazonalizada por Média Móvel
```{r}
decomp1.mm = decompose(desemp, type='multiplicative')

sazonal1.mm = decomp1.mm$seasonal
tendencia1.mm = decomp1.mm$trend
residuos1.mm = decomp1.mm$random

indice.saz1 = decomp1.mm$figure

dessaz.des = ts(desemp - sazonal1.mm, 
                start = c(1985,1), freq = 12)

par(mfrow=c(1,1))
plot(desemp, xlab = NULL, ylab = "Taxa de desemprego")
par(new=T)
plot(dessaz.des, col='blue',ann=F)
#Observa-se que a série dessazonalizada tem praticamente o mesma padrão da série original

Acf(dessaz.des, type="correlation",lag.max = 36)
Acf(dessaz.des, type="partial",lag.max = 36)
```

# Obtenção da série dessazonalizada por Dummies
```{r}
library(gets)

d=periodicdummies(desemp)

n = length(desemp)
trend = 1:n

d=ts(d)
X = cbind(1,trend,d[,1:11])

reg = lm(desemp[1:n]~X[1:n,]-1)
fit = ts(reg$fit,start=c(1985,1),frequency=12)

par(mfrow=c(1,1))
plot(desemp,ylim=range(desemp,fit))
lines(fit,col=2)
legend("bottomright",legend=c("Data","Fit"),col=1:2,lty=1)
...
```{r}
Acf(diff(diff(log(desemp),lag=1,differences=1),lag=12,differences=1),ylab="Diff 1",lag.max = 36)
```


```{r}

desemp.none=ur.df(diff(diff(log(desemp),lag=1,differences=1),lag=12,differences=1),type='none',lags=24,selectlags='BIC')
summary(desemp.none)
```
...
valor_param=2
dmat=(valor_param+1)^4 #valor_param+1, pois há três possibilidades para cada 
#ordem do modelo, caso valor_param=2;4 porque temos 4 ordens a serem determinadas.
...
for (i in 0:p_max)
  for (j in 0:q_max)
    for (k in 0:P_max)
      for (l in 0:Q_max){
        s = s + 1
        bics[s,] = c(i,j,k,l,Arima(window(desemp,end=c(2015,12)), 
...
```{r}
desemp.model1 = Arima(window(desemp,end=c(2015,12)), 
                                   order=c(2,1,2),seasonal=list(order=c(0,1,1),period=12), lambda=0)
...
```{r}
desemp.model2 = Arima(window(desemp,end=c(2015,12)), 
                                   order=c(2,1,2),seasonal=list(order=c(1,1,1),period=12), lambda=0)
